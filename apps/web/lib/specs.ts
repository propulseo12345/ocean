import type { ContentFormat, MediaType, Platform } from "@/lib/mocks/types"

// Specs médias par plateforme (état réel des API, juin 2026) + validation.
// Préfigure le Module J du PRD : messages FR, AVANT la programmation.

export type SpecSeverity = "error" | "warning"

export interface SpecIssue {
  severity: SpecSeverity
  message: string
}

/** Forme minimale validable — MediaAsset et LibraryAsset la satisfont. */
export interface MediaCheck {
  type: MediaType
  width: number
  height: number
  durationSec?: number
  fileSizeMb?: number
  mimeType?: string
}

export const IG_IMAGE_RATIO = { min: 0.8, max: 1.91 } // 4:5 à 1.91:1
export const IG_IMAGE_MAX_MB = 8
export const IG_REEL_DURATION = { min: 3, max: 900 } // 3 s à 15 min
export const REEL_MAX_MB = 300
export const FB_REEL_DURATION = { min: 3, max: 90 }
export const TIKTOK_DURATION = { min: 3, max: 600 }
export const STORY_MAX_SEC = 60
export const VERTICAL_RATIO = 9 / 16
export const CAROUSEL_LIMITS = { min: 2, max: 10 } // limite API Meta

const RATIO_TOLERANCE = 0.04
const JPEG_TYPES = ["image/jpeg", "image/jpg"]
const VIDEO_TYPES = ["video/mp4", "video/quicktime"]

/** Libellé lisible d'un ratio : "4:5", "1:1", "9:16", "1.91:1"… */
export function ratioLabel(width: number, height: number): string {
  const known: Array<[number, string]> = [
    [0.8, "4:5"],
    [1, "1:1"],
    [VERTICAL_RATIO, "9:16"],
    [1.91, "1.91:1"],
    [4 / 3, "4:3"],
    [3 / 4, "3:4"],
  ]
  const r = width / height
  const hit = known.find(([value]) => Math.abs(r - value) < RATIO_TOLERANCE)
  return hit ? hit[1] : `${(Math.round(r * 100) / 100).toLocaleString("fr-FR")}:1`
}

function issue(severity: SpecSeverity, message: string): SpecIssue {
  return { severity, message }
}

function checkVertical(media: MediaCheck, label: string): SpecIssue[] {
  const r = media.width / media.height
  if (Math.abs(r - VERTICAL_RATIO) > RATIO_TOLERANCE) {
    return [
      issue(
        "warning",
        `Recadrage 9:16 recommandé pour ${label} (actuel : ${ratioLabel(media.width, media.height)}).`
      ),
    ]
  }
  return []
}

function checkInstagramImage(media: MediaCheck): SpecIssue[] {
  const issues: SpecIssue[] = []
  const r = media.width / media.height
  if (r < IG_IMAGE_RATIO.min - RATIO_TOLERANCE || r > IG_IMAGE_RATIO.max + RATIO_TOLERANCE) {
    issues.push(
      issue(
        "error",
        `Ratio ${ratioLabel(media.width, media.height)} hors bornes Instagram (4:5 à 1.91:1) — recadre le visuel.`
      )
    )
  }
  if (media.fileSizeMb !== undefined && media.fileSizeMb > IG_IMAGE_MAX_MB) {
    issues.push(
      issue(
        "error",
        `Image de ${media.fileSizeMb.toLocaleString("fr-FR")} Mo — Instagram accepte ${IG_IMAGE_MAX_MB} Mo max en JPEG.`
      )
    )
  }
  if (media.mimeType && !JPEG_TYPES.includes(media.mimeType)) {
    issues.push(issue("warning", "Format non JPEG : conversion automatique avant publication."))
  }
  return issues
}

function checkVideo(
  media: MediaCheck,
  bounds: { min: number; max: number },
  platformLabel: string
): SpecIssue[] {
  const issues: SpecIssue[] = []
  if (media.type !== "video") {
    issues.push(issue("error", `Une vidéo est requise pour ce format sur ${platformLabel}.`))
    return issues
  }
  if (media.mimeType && !VIDEO_TYPES.includes(media.mimeType)) {
    issues.push(issue("error", "Format vidéo non supporté — MP4 ou MOV attendu."))
  }
  if (media.durationSec !== undefined) {
    if (media.durationSec < bounds.min) {
      issues.push(
        issue("error", `Vidéo trop courte : ${bounds.min} s minimum sur ${platformLabel}.`)
      )
    } else if (media.durationSec > bounds.max) {
      const label = bounds.max >= 60 ? `${Math.round(bounds.max / 60)} min` : `${bounds.max} s`
      issues.push(issue("error", `Vidéo trop longue : ${label} maximum sur ${platformLabel}.`))
    }
  }
  if (media.fileSizeMb !== undefined && media.fileSizeMb > REEL_MAX_MB) {
    issues.push(
      issue(
        "error",
        `Vidéo de ${media.fileSizeMb.toLocaleString("fr-FR")} Mo — ${REEL_MAX_MB} Mo maximum.`
      )
    )
  }
  return issues
}

/** Valide un média pour une plateforme et un format ciblés → issues FR. */
export function validateMedia(
  media: MediaCheck,
  platform: Platform,
  format: ContentFormat
): SpecIssue[] {
  if (platform === "newsletter" || platform === "custom") return []

  if (format === "reel") {
    const bounds =
      platform === "tiktok"
        ? TIKTOK_DURATION
        : platform === "facebook"
          ? FB_REEL_DURATION
          : IG_REEL_DURATION
    const label =
      platform === "tiktok" ? "TikTok" : platform === "facebook" ? "Facebook" : "Instagram"
    return [...checkVideo(media, bounds, label), ...checkVertical(media, "un Reel")]
  }

  if (format === "story") {
    const issues = checkVertical(media, "une story")
    if (
      media.type === "video" &&
      media.durationSec !== undefined &&
      media.durationSec > STORY_MAX_SEC
    ) {
      issues.push(issue("error", `Story vidéo limitée à ${STORY_MAX_SEC} s.`))
    }
    return issues
  }

  // post / carousel
  if (media.type === "video") {
    return platform === "instagram"
      ? [issue("warning", "Les vidéos du feed sont publiées comme Reels par Instagram.")]
      : []
  }
  return platform === "instagram" ? checkInstagramImage(media) : []
}

/** Valide le nombre de visuels d'un carrousel (limite API Meta : 10). */
export function validateCarousel(slideCount: number): SpecIssue[] {
  if (slideCount > CAROUSEL_LIMITS.max) {
    return [
      issue("error", `Maximum ${CAROUSEL_LIMITS.max} visuels par carrousel (limite API Meta).`),
    ]
  }
  if (slideCount < CAROUSEL_LIMITS.min) {
    return [issue("warning", `Un carrousel contient au moins ${CAROUSEL_LIMITS.min} visuels.`)]
  }
  return []
}
