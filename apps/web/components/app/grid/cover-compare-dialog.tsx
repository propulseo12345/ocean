...
export function CoverCompareDialog({
  ...
  function pick(url: string, kept: boolean) {
    ...
    onClose()
    toast.success(
      kept ? "Cover actuelle conservée (aperçu)" : "Cover alternative appliquée (aperçu)",
      { description: "Le choix réel se fera dans la fiche studio du Reel." }
    )
  }
  ...
  return (
    <Dialog open={target !== null} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Tester la cover du Reel</DialogTitle>
          <DialogDescription>
            {target?.tile.title} — chaque option est montrée entre ses voisines de grille.
          </DialogDescription>
        </DialogHeader>
        {target ? (
          <div className="grid grid-cols-2 gap-4">
            <CoverOption
              label="Cover actuelle"
              url={target.current}
              target={target}
              actionLabel="Garder celle-ci"
              onSelect={() => pick(target.current, true)}
            />
            <CoverOption
              label="Alternative (1re frame)"
              url={target.alternative}
              target={target}
              actionLabel="Choisir celle-ci"
              primary
